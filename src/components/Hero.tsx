...
const SWING = { x: 26, y: 20 }; // px of travel at depth 1, on a full desktop canvas

/** Nameplate fluid scale — width-driven, but capped by viewport *height* so
 *  landscape phones and low desktops never collide with the enso above. */
const NAME_FLUID = "clamp(2.15rem, min(9.8vw, 11.5vh), 8.6rem)";
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const onPointerMove = (event: PointerEvent) => {
      ...
      if (crossXRef.current) crossXRef.current.style.transform = `translate3d(${localX}px,0,0)`;
      if (crossYRef.current) crossYRef.current.style.transform = `translate3d(0,${localY}px,0)`;
      if (coordRef.current)
    ...
    const tick = () => {
      ...
      // Swing intensity scales with the canvas: full at ≥1440px, ~45% on small windows.
      const scale = Math.min(1, Math.max(0.45, window.innerWidth / 1440));
      ...
      const setLayer = (
        ...
        if (!el) return;
        const x = current.x * SWING.x * depth * scale;
        const y = current.y * SWING.y * depth * scale + window.scrollY * scrollFactor;
        el.style.transform = `translate3d(${x.toFixed(2)}px, ${y.toFixed(2)}px, 0)${
          tilt ? ` rotate(${(current.x * tilt * scale).toFixed(2)}deg)` : ""
        }`;
      };
  ...
  return (
    <section id="top" ref={sectionRef} className="relative min-h-svh overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(120% 90% at 70% 20%, transparent 40%, rgba(12,12,14,0.75) 100%)",
        }}
        aria-hidden="true"
      />
      ...
      {/* ---------- interactive object ----------
          ...
          Tablet : reduced + repositioned, deeper bleed keeps it clear of the nameplate
          Desktop: owns the upper-right territory, height-capped for low viewports */}
      <div
        className="pointer-events-auto absolute right-[-16%] top-[8%] z-10 w-[56vw] max-w-[240px]
          sm:right-[-10%] sm:top-[9%] sm:w-[46vw] sm:max-w-[280px]
          md:right-[-8%] md:top-[12%] md:w-[44vw] md:max-w-[380px]
          lg:right-[4%] lg:top-[15%] lg:w-[min(30vw,46vh)] lg:max-w-[540px]"
      >
        <EnsoObject outerRef={ensoOuterRef} coreRef={ensoCoreRef} />
      </div>

      {/* ---------- identity block ---------- */}
      <div className="shell relative z-20 flex min-h-svh flex-col justify-end pb-8 pt-24 md:pb-14 md:pt-28 lg:pb-12">
        {/* roles overline */}
        <div className="reveal-mask mb-4">
          <p
            className="reveal-line flex max-w-[62vw] flex-wrap items-center gap-x-3 gap-y-1 text-[11px] uppercase tracking-[0.22em] text-paper-dim sm:max-w-none md:text-xs"
        ...
        <div className="reveal-mask mt-6 md:mt-8">
          <p
            className="reveal-line max-w-xl font-mono text-[13px] leading-relaxed text-paper-dim md:text-[15px]"
            style={{ animationDelay: "0.62s" }}
          >
            Designing interfaces<span className="text-denki">.</span> Building digital
            experiences<span className="text-denki">.</span>
            <span lang="ja" className="ml-3 inline-block font-jp text-xs text-paper-faint">
              設計と構築
            </span>
          </p>
        </div>
        ...
        <div
          className="fade-in mt-10 flex flex-wrap items-end justify-between gap-x-6 gap-y-5 border-t border-line pt-6 md:mt-16 md:gap-x-10"
          style={{ animationDelay: "0.85s" }}
        >
          <a
            href="#work"
            className="group -ml-2 flex items-center gap-4 py-3 pl-2 pr-1 text-xs uppercase tracking-[0.2em] text-paper transition-colors duration-300 hover:text-denki focus-visible:text-denki md:text-sm"
          >
            <span className="font-mono text-[10px] text-paper-faint transition-colors duration-300 group-hover:text-denki">
              01
            </span>
            View selected work
            <span className="relative flex h-px w-10 items-center bg-paper transition-all duration-500 group-hover:w-16 group-hover:bg-denki md:w-12 md:group-hover:w-20">
              <svg
                viewBox="0 0 10 10"
